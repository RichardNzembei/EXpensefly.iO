...
const handleFormSubmit = (e) => {
    ...
    activeTargets.value.push(newTarget);
    email.value = '';
...
const markAsCompleted = (index) => {
    const completedTarget = activeTargets.value.splice(index, 1)[0];
    completedTargets.value.push(completedTarget);
};
</script>
    ...
    <section class="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg  space-y-8">
        ...
        <form @submit="handleFormSubmit" class="space-y-4">
            ...
            <div>
                ...
                <input v-model="targetAmount" v-numeric-only type="number"
                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <section class="mt-8">
            ...
            <div v-if="activeTargets.length" class="space-y-4">
                <div v-for="(target, index) in activeTargets" :key="index"
                    ...
                    <p class="text-sm text-gray-600">Goal: ${{ target.amount }} | Deadline: {{ target.deadline }}</p>
        ...
        <section class="mt-8">
            ...
            <div v-if="completedTargets.length" class="space-y-4">
                <div v-for="(target, index) in completedTargets" :key="index"
                    class="p-4 border border-green-200 rounded-md bg-green-50">
                    <h3 class="text-lg font-bold text-gray-800">{{ target.description }}</h3>
                    <p class="text-sm text-gray-600">Goal: ${{ target.amount }} | Completed on: {{ target.deadline }}
                    </p>
                </div>
            </div>
